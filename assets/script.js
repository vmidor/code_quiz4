...
function nextQuestion() {
    ...
    // takes the answer options from the current index in the questions array and assigns them one by one to each of the answer buttons (could be made into a for loop if there were more questions)
...
function checkAnswer(event) {
    ...
    if (event.target.textContent === questions[currentQuestion].answer) {
        ...
        // ends game if timer is less than 0 seconds, as 0 seconds will be removed
        if (timerSecs < 0) {
            timerSecs -= 0;
            endGame();
